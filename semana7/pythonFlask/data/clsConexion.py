...
class clsConexion:
    ...
    _servidor = "localhost"  # Recuerda cambiar la dirección y contraseña
    _basedatos = "datos"
    _usuario = "sa"
    _contra = "12345678*"
    ...
    def agregar(self, dato):
        estado = False
        AuxSql = "insert into datos(texto, descripcion) values('{0}','{1}')".format(
            dato.Texto, dato.Descripcion
        )
        try:
    ...
    def editar(self, dato):
        estado = False
        AuxSql = (
            "update datos set texto = '{1}', descripcion = '{2}' where id = {0}".format(
                dato.ID, dato.Texto, dato.Descripcion
            )
        )
        try:
            _conex = self._conectar()
